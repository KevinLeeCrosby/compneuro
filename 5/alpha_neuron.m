function output_spike_count = alpha_neuron(t_peak)
...
% E_syn = +1; % Excitatory synapse
% E_syn = 0; % Excitatory synapse (why is this excitatory?)
E_syn = V_th + 5; % Excitatory synapse
% E_syn = V_th - 1; % Inhibitory synapse
% E_syn = E_leak - 1; % Inhibitory synapse
...
ttl = sprintf('Output spike train for t_{peak} = %.1f', t_peak);
title(ttl);
% title('Output spike train');
drawnow

output_spike_count = nnz(V_trace > V_th);